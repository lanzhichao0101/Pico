...
//#region node_modules/.pnpm/@webspatial+core-sdk@1.6.1/node_modules/@webspatial/core-sdk/dist/index.js
...
var isSSR, isSSREnv;
var init_ssr_polyfill = __esm({ "src/ssr-polyfill.ts"() {
	"use strict";
	isSSR = typeof window === "undefined";
	isSSREnv = () => isSSR;
} });
var SSRPlatform;
var init_SSRPlatform = __esm({ "src/platform-adapter/ssr/SSRPlatform.ts"() {
	"use strict";
	SSRPlatform = class {
		callJSB(cmd, msg) {
...
var init_CommandResultUtils = __esm({ "src/platform-adapter/CommandResultUtils.ts"() {
	"use strict";
} });
var PuppeteerPlatform_exports = {};
__export(PuppeteerPlatform_exports, { PuppeteerPlatform: () => PuppeteerPlatform });
var PuppeteerPlatform;
var init_PuppeteerPlatform = __esm({ "src/platform-adapter/puppeteer/PuppeteerPlatform.ts"() {
	"use strict";
	...
	PuppeteerPlatform = class {
		...
		callJSB(cmd, msg) {
			return new Promise((resolve) => {
				try {
					if (window.__handleJSBMessage) try {
						console.log(` core-sdk Puppeteer Platform: callJSB: ${cmd}::${msg}`);
						const result = window.__handleJSBMessage(`${cmd}::${msg}`);
						console.log(` core-sdk Puppeteer Platform callJSB result: ${result}`);
						resolve(CommandResultSuccess(result));
					} catch (err) {
						resolve(CommandResultFailure("500", "JSB execution error"));
					}
					else resolve(CommandResultSuccess("ok"));
				} catch (error) {
					console.error(`PuppeteerPlatform cmd Error: ${cmd}, msg: ${msg} error: ${error}`);
		...
		*/
		createSpatializedElementSync(spatialId, webspatialUrl) {
			try {
				console.log(`[Puppeteer Platform] Creating spatialized element sync with id: ${spatialId}, url: ${webspatialUrl}`);
				const win = window;
				if (win.__handleJSBMessage) {
					...
					win.__handleJSBMessage(`CreateSpatialized2DElement::${JSON.stringify(createCommand)}`);
				}
			} catch (error) {
		...
		callWebSpatialProtocol(command, query, target, features) {
			console.log(`PuppeteerPlatform: Calling webspatial protocol: webspatial://${command}${query ? `?${query}` : ""}`);
			return new Promise((resolve) => {
				try {
					const webspatialUrl = `webspatial://${command}${query ? `?${query}` : ""}`;
					const { spatialId, iframe, windowProxy } = this.createIframeWindow(webspatialUrl, target, features);
					if (command === "createSpatialized2DElement") this.createSpatializedElementSync(spatialId, webspatialUrl);
					console.log(`[Puppeteer Platform] iframe created with spatialId: ${spatialId}`);
					this.iframeRegistry.set(spatialId, iframe);
					resolve(CommandResultSuccess({
						windowProxy,
						id: spatialId
					}));
				} catch (error) {
					console.error("Error calling webspatial protocol:", error);
					resolve(CommandResultFailure("500", "Failed to call webspatial protocol"));
				}
			});
		}
		callWebSpatialProtocolSync(command, query, target, features) {
			try {
				...
				return CommandResultFailure("500", "Failed to call webspatial protocol sync");
			}
		}
		/**
		...
		*/
		createIframeWindow(url, target, features) {
			...
			if (featuresObj.width) iframe.style.width = featuresObj.width;
			if (featuresObj.height) iframe.style.height = featuresObj.height;
			if (featuresObj.left) {
			...
			console.log(`PuppeteerPlatform created iframe window with spatialId: ${spatialId}, URL: ${url}`);
		...
		*/
		createEnhancedWindowProxy(iframe, url, spatialId) {
			return {
				...
				navigator: { userAgent: `Mozilla/5.0 (WebKit) SpatialId/${spatialId}` },
				close: () => {
				...
				postMessage: (message, targetOrigin) => {
					if (iframe.contentWindow) iframe.contentWindow.postMessage(message, targetOrigin || "*");
				},
				addEventListener: (type, listener) => {
					if (iframe.contentWindow) iframe.contentWindow.addEventListener(type, listener);
				},
				removeEventListener: (type, listener) => {
					if (iframe.contentWindow) iframe.contentWindow.removeEventListener(type, listener);
				},
				executeScript: (code) => {
					if (iframe.contentWindow) try {
						return iframe.contentWindow.eval(code);
					} catch (error) {
						console.error(`Error executing script in iframe ${spatialId}:`, error);
						return null;
					}
					return null;
				},
...
var SpatialWebEvent;
var init_SpatialWebEvent = __esm({ "src/SpatialWebEvent.ts"() {
	"use strict";
...
__export(PicoOSPlatform_exports, { PicoOSPlatform: () => PicoOSPlatform });
function nextRequestId() {
	requestId = (requestId + 1) % MAX_ID;
	return `rId_${requestId}`;
}
var requestId, MAX_ID, PicoOSPlatform;
var init_PicoOSPlatform = __esm({ "src/platform-adapter/pico-os/PicoOSPlatform.ts"() {
	"use strict";
	init_CommandResultUtils();
	...
	PicoOSPlatform = class {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					SpatialWebEvent.addEventReceiver(rId, (result) => {
						...
						if (result.success) resolve(CommandResultSuccess(result.data));
						else {
		...
		async callWebSpatialProtocol(command, query, target, features) {
			return new Promise((resolve, reject) => {
				...
				try {
					...
					SpatialWebEvent.addEventReceiver(createdId, (result) => {
						resolve(CommandResultSuccess({
							windowProxy,
							id: result.spatialId
						}));
					...
					windowProxy = this.openWindow(command, "rid=" + createdId, target, features).windowProxy;
				} catch (error) {
					const { code, message } = error;
					SpatialWebEvent.removeEventReceiver(createdId);
					resolve(CommandResultFailure(code, message));
		...
		callWebSpatialProtocolSync(command, query, target, features) {
			const { spatialId: id = "", windowProxy } = this.openWindow(command, query, target, features);
			return CommandResultSuccess({
				windowProxy,
				id
			});
		...
		openWindow(command, query, target, features) {
			return {
				spatialId: "",
				windowProxy: window.open(`webspatial://${command}?${query || ""}`, target, features)
			};
		}
	};
} });
var AndroidPlatform_exports = {};
__export(AndroidPlatform_exports, { AndroidPlatform: () => AndroidPlatform });
...
var init_AndroidPlatform = __esm({ "src/platform-adapter/android/AndroidPlatform.ts"() {
	"use strict";
	init_CommandResultUtils();
	init_JSBCommand();
	init_SpatialWebEvent();
	...
	AndroidPlatform = class {
		...
		async callWebSpatialProtocol(command, query, target, features) {
			...
			while (!windowProxy?.open) await new Promise((resolve) => setTimeout(resolve, 16));
			windowProxy?.open("about:blank", "_self");
			while (!windowProxy?.__SpatialId) await new Promise((resolve) => setTimeout(resolve, 16));
...
__export(VisionOSPlatform_exports, { VisionOSPlatform: () => VisionOSPlatform });
var VisionOSPlatform;
var init_VisionOSPlatform = __esm({ "src/platform-adapter/vision-os/VisionOSPlatform.ts"() {
	"use strict";
	init_CommandResultUtils();
	VisionOSPlatform = class {
		async callJSB(cmd, msg) {
			try {
				return CommandResultSuccess(await window.webkit.messageHandlers.bridge.postMessage(`${cmd}::${msg}`));
			} catch (error) {
		...
		openWindow(command, query, target, features) {
			const windowProxy = window.open(`webspatial://${command}?${query || ""}`, target, features);
			return {
				spatialId: (windowProxy?.navigator.userAgent)?.match(/\b([0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12})\b/gi)?.[0],
				windowProxy
			};
		}
	};
...
function createPlatform() {
	if (isSSREnv()) return new SSRPlatform();
	...
	} else if (userAgent.includes("PicoWebApp") && isVersionGreater(webSpatialVersion, [
		0,
		0,
		1
	])) {
...
var init_platform_adapter = __esm({ "src/platform-adapter/index.ts"() {
	"use strict";
	init_ssr_polyfill();
	init_SSRPlatform();
} });
function parseBorderRadius$1(borderProperty, width) {
	if (borderProperty === "") return 0;
	if (borderProperty.endsWith("%")) return width * parseFloat(borderProperty) / 100;
	return parseFloat(borderProperty);
}
function parseCornerRadius$1(computedStyle) {
	const width = parseFloat(computedStyle.getPropertyValue("width"));
	const topLeftPropertyValue = computedStyle.getPropertyValue("border-top-left-radius");
	const topRightPropertyValue = computedStyle.getPropertyValue("border-top-right-radius");
	const bottomLeftPropertyValue = computedStyle.getPropertyValue("border-bottom-left-radius");
	const bottomRightPropertyValue = computedStyle.getPropertyValue("border-bottom-right-radius");
	return {
		topLeading: parseBorderRadius$1(topLeftPropertyValue, width),
		bottomLeading: parseBorderRadius$1(bottomLeftPropertyValue, width),
		topTrailing: parseBorderRadius$1(topRightPropertyValue, width),
		bottomTrailing: parseBorderRadius$1(bottomRightPropertyValue, width)
	};
}
...
var init_JSBCommand = __esm({ "src/JSBCommand.ts"() {
	"use strict";
	...
	UpdateEntityPropertiesCommand = class extends JSBCommand {
		...
		getParams() {
			const transform = composeSRT(this.properties.position ?? this.entity.position, this.properties.rotation ?? this.entity.rotation, this.properties.scale ?? this.entity.scale).toFloat64Array();
			return {
	...
	CreateModelComponentCommand = class extends JSBCommand {
		...
		getParams() {
			return {
				geometryId: this.options.mesh.id,
				materialIds: this.options.materials.map((material) => material.id)
			};
		}
	...
	WebSpatialProtocolCommand = class extends JSBCommand {
		...
		async execute() {
			...
			return platform.callWebSpatialProtocol(this.commandType, query, this.target, this.features);
		}
		executeSync() {
			const query = this.getQuery();
			return platform.callWebSpatialProtocolSync(this.commandType, query, this.target, this.features);
		}
		getQuery() {
			let query = void 0;
			const params = this.getParams();
			if (params) query = Object.keys(params).map((key) => {
...
var SpatialScene = class _SpatialScene extends SpatialObject {
	...
	*/
	static getInstance() {
		if (!instance) instance = new _SpatialScene("");
		return instance;
	}
	async convertCoordinate(position, fromId, toId) {
		try {
			return (await new ConvertCoordinateCommand(position, fromId, toId).execute())?.data ?? position;
		} catch (error) {
...
function isValidSceneUnit(val) {
	...
	if (typeof val === "string") {
		if (val.endsWith("px")) {
			if (isNaN(Number(val.slice(0, -2)))) return false;
			return Number(val.slice(0, -2)) >= 0;
		}
		if (val.endsWith("m")) {
			if (isNaN(Number(val.slice(0, -1)))) return false;
...
function pointToPhysical(point, options) {
	updateValue();
	if (getWorldScalingCompensation(options) === "unscaled") return point / snapshot.meterToPtUnscaled;
	return point / snapshot.meterToPtScaled;
}
function physicalToPoint(physical, options) {
	updateValue();
	if (getWorldScalingCompensation(options) === "unscaled") return physical * snapshot.meterToPtUnscaled;
...
function updateValue() {
	...
	if (next.meterToPtScaled !== snapshot.meterToPtScaled || next.meterToPtUnscaled !== snapshot.meterToPtUnscaled) snapshot = next;
}
function getValue() {
	updateValue();
	return snapshot;
...
var defaultSceneConfig = { defaultSize: {
	width: 1280,
	height: 720
} };
var defaultSceneConfigVolume = { defaultSize: {
	width: "0.94m",
	height: "0.94m",
	depth: "0.94m"
} };
var xr_window_defaults = { ...defaultSceneConfig };
var xr_volume_defaults = { ...defaultSceneConfigVolume };
...
function deepMergePlain(base, over) {
	...
	for (const k of Object.keys(over)) {
		const bv = out[k];
		const ov = over[k];
		if (ov && typeof ov === "object" && !Array.isArray(ov) && bv && typeof bv === "object" && !Array.isArray(bv)) out[k] = deepMergePlain(bv, ov);
		else out[k] = ov;
	}
	return out;
}
function normalizeXRDefaultsToSceneOptions(src) {
	...
	if (ds !== void 0) out.defaultSize = ds;
	if ("default_size" in out) delete out.default_size;
	return out;
}
var SceneManager = class _SceneManager {
	...
	manifestReady = null;
	static getInstance() {
		if (!_SceneManager.instance) _SceneManager.instance = new _SceneManager();
	...
	async setupManifest() {
		...
		try {
			...
			if (windowNext && Object.keys(windowNext).length > 0) xr_window_defaults = windowNext;
			if (volumeNext && Object.keys(volumeNext).length > 0) xr_volume_defaults = volumeNext;
		} catch (error) {
			console.warn("SceneManager.setupManifest failed; using built-in defaults.", error?.message || error);
		}
	}
	open = (url, target, features) => {
		if (url?.startsWith(INTERNAL_SCHEMA_PREFIX)) {
			if (url.includes("createSpatialized2DElement") || url.includes("createAttachment")) {
				const token = (window.webSpatial || window.__webspatialShell__)?.genToken?.();
				if (token) {
					const command = url.includes("createAttachment") ? "createAttachment" : "createSpatialized2DElement";
					const host = window.location.host;
					const finalURL = `${window.location.protocol}//${host}/${token}/?command=${command}`;
		...
		if (target === "_self" || target === "_parent" || target === "_top") return this.originalOpen(url, target, features);
		let cfg = target ? this.getConfig(target) : void 0;
		if (cfg === void 0) {
			const [ans] = formatSceneConfig(deepCloneJSON(getSceneDefaultConfig("window")), "window");
			cfg = {
				...ans,
				type: "window"
			};
		}
		const result = new createSpatialSceneCommand(url, cfg, target, features).executeSync();
		const id = result.data?.id;
		if (id) new FocusScene(id).execute();
	...
	initScene(name, callback, options) {
		...
		const sanitizedReturnVal = sanitizeSceneOptionsUnits(deepCloneJSON(callback(this.callbackReturnMap[name] ?? (() => {
			return deepCloneJSON(defaultConfigRaw);
		})())));
		const clonedForFormat = deepCloneJSON(sanitizedReturnVal);
		const [formattedConfig, errors] = formatSceneConfig(deepMergePlain(deepCloneJSON(getSceneDefaultConfig(sceneType)), clonedForFormat), sceneType);
		if (errors.length > 0) console.warn(`initScene ${name} with errors: ${errors.join(", ")}`);
	...
	*/
	async getPWAManifest(manifestUrl) {
		...
		if (!href) return;
		if (href.startsWith("data:")) try {
			const comma = href.indexOf(",");
			if (comma < 0) return;
			const meta = href.slice(5, comma);
			const data = href.slice(comma + 1);
			const decoded = /;base64/i.test(meta) ? atob(data) : decodeURIComponent(data);
...
function sanitizeSceneOptionsUnits(val) {
	if (val?.defaultSize) {
		for (const k of [
			...
			"height",
			"depth"
		]) if (k in val.defaultSize && !isValidSceneUnit(val.defaultSize[k])) delete val.defaultSize[k];
	}
	if (val?.resizability) {
		for (const k of [
			"minWidth",
			...
			"maxWidth",
			"maxHeight"
		]) if (k in val.resizability && !isValidSceneUnit(val.resizability[k])) delete val.resizability[k];
	}
	return val;
}
...
function formatToNumber(str, targetUnit, defaultUnit) {
	if (typeof str === "number") {
		if (defaultUnit === "px" && targetUnit === "px" || defaultUnit === "m" && targetUnit === "m") return str;
		if (defaultUnit === "px" && targetUnit === "m") return pxToMeter(str);
		else if (defaultUnit === "m" && targetUnit === "px") return meterToPx(str);
		return str;
	}
	if (targetUnit === "m") if (str.endsWith("m")) return Number(str.slice(0, -1));
	else if (str.endsWith("px")) return pxToMeter(Number(str.slice(0, -2)));
	else throw new Error("formatToNumber: invalid str");
	else if (targetUnit === "px") if (str.endsWith("px")) return Number(str.slice(0, -2));
	else if (str.endsWith("m")) return meterToPx(Number(str.slice(0, -1)));
	else throw new Error("formatToNumber: invalid str");
	else throw new Error("formatToNumber: invalid targetUnit");
}
function formatSceneConfig(config, sceneType) {
	const errors = [];
	const isWindow = sceneType === "window";
	if (!isValidSpatialSceneType(sceneType)) errors.push(`sceneType`);
	if (config.defaultSize) for (let k of [
		"width",
		"height",
		"depth"
	]) {
		if (!(k in config.defaultSize)) continue;
		if (isValidSceneUnit(config.defaultSize[k])) config.defaultSize[k] = formatToNumber(config.defaultSize[k], isWindow ? "px" : "m", "px");
		else {
			delete config.defaultSize[k];
			errors.push(`defaultSize.${k}`);
		}
	}
	if (config.resizability) for (let k of [
		"minWidth",
		"minHeight",
		"maxWidth",
		"maxHeight"
		...
		if (isValidSceneUnit(config.resizability[k])) config.resizability[k] = formatToNumber(config.resizability[k], "px", "px");
		else {
	...
	openedWindow.document.onclick = function(e) {
		...
		while (element) {
			if (element.tagName == "A") {
				if (handleATag(e, element)) return false;
				return true;
			}
...
async function injectScenePolyfill() {
	...
	if (await SpatialScene.getInstance().getState() !== "pending") return;
	...
	onContentLoaded(async () => {
		...
		let cfg = pre;
		if (typeof window.xrCurrentSceneDefaults === "function") try {
		...
		const [formattedConfig, errors] = formatSceneConfig(deepMergePlain(deepCloneJSON(rawDefault), cfg), sceneType);
		if (errors.length > 0) console.warn(`window.xrCurrentSceneDefaults with errors: ${errors.join(", ")}`);
		const finalCfg = {
			...formattedConfig,
			type: sceneType
		};
...
var SpatializedElement = class extends SpatialObject {
	...
	*/
	onReceiveEvent(data) {
		const { type } = data;
		if (type === "objectdestroy") this.isDestroyed = true;
		else if (type === "spatialtap") {
			const event = createSpatialEvent("spatialtap", data.detail);
			this._onSpatialTap?.(event);
		} else if (type === "spatialdragstart") {
			const dragStartEvent = createSpatialEvent("spatialdragstart", data.detail);
			this._onSpatialDragStart?.(dragStartEvent);
		} else if (type === "spatialdrag") {
			const event = createSpatialEvent("spatialdrag", data.detail);
			this._onSpatialDrag?.(event);
		} else if (type === "spatialdragend") {
			const event = createSpatialEvent("spatialdragend", data.detail);
			this._onSpatialDragEnd?.(event);
		} else if (type === "spatialrotate") {
			const event = createSpatialEvent("spatialrotate", data.detail);
			this._onSpatialRotate?.(event);
		} else if (type === "spatialrotateend") {
			const event = createSpatialEvent("spatialrotateend", data.detail);
			this._onSpatialRotateEnd?.(event);
		} else if (type === "spatialmagnify") {
			const event = createSpatialEvent("spatialmagnify", data.detail);
			this._onSpatialMagnify?.(event);
		} else if (type === "spatialmagnifyend") {
			const event = createSpatialEvent("spatialmagnifyend", data.detail);
	...
	set onSpatialTap(value) {
		...
		this.updateProperties({ enableTapGesture: value !== void 0 });
	}
	_onSpatialDragStart;
	set onSpatialDragStart(value) {
		this._onSpatialDragStart = value;
		this.updateProperties({ enableDragStartGesture: this._onSpatialDragStart !== void 0 });
	}
	_onSpatialDrag;
	set onSpatialDrag(value) {
		this._onSpatialDrag = value;
		this.updateProperties({ enableDragGesture: this._onSpatialDrag !== void 0 });
	}
	_onSpatialDragEnd;
	set onSpatialDragEnd(value) {
		this._onSpatialDragEnd = value;
		this.updateProperties({ enableDragEndGesture: value !== void 0 });
	}
	_onSpatialRotate;
	set onSpatialRotate(value) {
		this._onSpatialRotate = value;
		this.updateProperties({ enableRotateGesture: this._onSpatialRotate !== void 0 });
	}
	_onSpatialRotateEnd;
	set onSpatialRotateEnd(value) {
		this._onSpatialRotateEnd = value;
		this.updateProperties({ enableRotateEndGesture: value !== void 0 });
	}
	_onSpatialMagnify;
	set onSpatialMagnify(value) {
		this._onSpatialMagnify = value;
		this.updateProperties({ enableMagnifyGesture: value !== void 0 });
	}
	_onSpatialMagnifyEnd;
	set onSpatialMagnifyEnd(value) {
		this._onSpatialMagnifyEnd = value;
		this.updateProperties({ enableMagnifyEndGesture: value !== void 0 });
	}
	/**
...
var Spatialized2DElement = class extends SpatializedElement {
	...
	*/
	async addSpatializedElement(element) {
		return new AddSpatializedElementToSpatialized2DElement(this, element).execute();
	}
};
init_JSBCommand();
	...
	*/
	async updateProperties(properties) {
		...
		if (properties.sources !== void 0) {
			if (JSON.stringify(this.sources) !== JSON.stringify(properties.sources)) {
				this.sources = properties.sources;
				needsReadyReset = true;
			}
		}
		if (needsReadyReset) this.ready = this.createReadyPromise();
		if (properties.autoplay !== void 0) this._autoplay = properties.autoplay;
		if (properties.loop !== void 0) this._loop = properties.loop;
		if (properties.playbackRate !== void 0) this._playbackRate = properties.playbackRate;
		return new UpdateSpatializedStatic3DElementProperties(this, properties).execute();
	}
	/**
...
var SpatializedDynamic3DElement = class extends SpatializedElement {
	...
	async updateProperties(properties) {
		return new UpdateSpatializedDynamic3DElementProperties(this, properties).execute();
	}
};
async function createSpatialized2DElement() {
	const result = await new createSpatialized2DElementCommand().execute();
	if (!result.success) throw new Error("createSpatialized2DElement failed");
	else {
...
async function createSpatializedStatic3DElement(modelURL, sources) {
	const result = await new CreateSpatializedStatic3DElementCommand(modelURL, sources).execute();
	if (!result.success) throw new Error("createSpatializedStatic3DElement failed");
	else {
...
async function createSpatializedDynamic3DElement() {
	...
	if (!result.success) throw new Error("createSpatializedDynamic3DElement failed");
	else {
		const { id } = result.data;
...
async function createAttachmentEntity(options) {
	...
	if (!result.success) throw new Error("createAttachmentEntity failed: " + result?.errorMessage);
	const { id, windowProxy } = result.data;
...
var SpatialEntity = class extends SpatialObject {
	...
	set enableInput(value) {
		...
		this.updateEntityEvent("spatialtap", value).catch((err) => {
			console.error("enableInput updateEntityEvent failed", "spatialtap", err);
			if (this._enableInput === value) this._enableInput = !value;
		});
	}
	...
	async removeFromParent() {
		const ans = await new SetParentForEntityCommand(this.id, void 0).execute();
		if (this.parent) {
	...
	onReceiveEvent = (data) => {
		...
		if (type === "objectdestroy") this.isDestroyed = true;
			...
			const evt = createSpatialEvent("spatialtap", data.detail);
			this.dispatchEvent(evt);
		} else if (type === "spatialdragstart") {
			const evt = createSpatialEvent("spatialdragstart", data.detail);
			this.dispatchEvent(evt);
		} else if (type === "spatialdrag") {
			const evt = createSpatialEvent("spatialdrag", data.detail);
			this.dispatchEvent(evt);
		} else if (type === "spatialdragend") {
			const evt = createSpatialEvent("spatialdragend", data.detail);
			this.dispatchEvent(evt);
		} else if (type === "spatialrotate") {
			const evt = createSpatialEvent("spatialrotate", data.detail);
			this.dispatchEvent(evt);
		} else if (type === "spatialrotateend") {
			const evt = createSpatialEvent("spatialrotateend", data.detail);
			this.dispatchEvent(evt);
		} else if (type === "spatialmagnify") {
			const evt = createSpatialEvent("spatialmagnify", data.detail);
			this.dispatchEvent(evt);
		} else if (type === "spatialmagnifyend") {
			const evt = createSpatialEvent("spatialmagnifyend", data.detail);
			this.dispatchEvent(evt);
		}
	};
	dispatchEvent(evt) {
		if (!evt.__origin) Object.defineProperty(evt, "__origin", {
			value: this,
			enumerable: false
		});
		this.events[evt.type]?.(evt);
	...
	async convertFromEntityToEntity(fromEntityId, toEntityId, position) {
		return new ConvertFromEntityToEntityCommand(fromEntityId, toEntityId, position).execute();
	}
...
async function createSpatialEntity(userData) {
	...
	if (!result.success) throw new Error("createSpatialEntity failed:" + result?.errorMessage);
	else {
		const { id } = result.data;
...
async function createSpatialGeometry(ctor, options) {
	const result = await new CreateSpatialGeometryCommand(ctor.type, options).execute();
	if (!result.success) throw new Error("createSpatialGeometry failed:" + result?.errorMessage);
	else {
		const { id } = result.data;
...
async function createSpatialUnlitMaterial(options) {
	...
	if (!result.success) throw new Error("createSpatialUnlitMaterial failed:" + result?.errorMessage);
	else {
		const { id } = result.data;
...
async function createModelComponent(options) {
	...
	if (!result.success) throw new Error("createModelComponent failed:" + result?.errorMessage);
	else {
		const { id } = result.data;
...
async function createSpatialModelEntity(options, userData) {
	...
	if (!result.success) throw new Error("createSpatialModelEntity failed:" + result?.errorMessage);
	else {
		const { id } = result.data;
...
async function createModelAsset(options) {
	...
	if (!result.success) throw new Error("createModelAsset failed:" + result?.errorMessage);
	else {
		const { id } = result.data;
...
var Spatial = class {
	...
	*/
	runInSpatialWeb() {
		if (navigator.userAgent.indexOf("WebSpatial/") >= 0) return true;
		return false;
	}
	getShellVersionFromUA() {
		if (this.wsAppShellVersionFromUA !== void 0) return this.wsAppShellVersionFromUA;
		if (typeof navigator === "undefined" || typeof navigator.userAgent !== "string") {
			this.wsAppShellVersionFromUA = null;
			return null;
		}
		const match = navigator.userAgent.match(/WSAppShell\/(\d+(?:\.\d+){2}(?:[-+][0-9A-Za-z.-]+)*)/);
	...
	*/
	getNativeVersion() {
		if (window.__WebSpatialData && window.__WebSpatialData.getNativeVersion) return window.__WebSpatialData.getNativeVersion();
...
init_ssr_polyfill();
init_utils();
var spatial$1 = new Spatial();
var session = void 0;
var SpatialGlobalCustomVars = { backgroundMaterial: "--xr-background-material" };
var htmlBackgroundMaterial = "";
function setCurrentWindowStyle(backgroundMaterial) {
	if (backgroundMaterial !== htmlBackgroundMaterial) {
		session?.getSpatialScene()?.updateSpatialProperties({ material: backgroundMaterial });
		htmlBackgroundMaterial = backgroundMaterial;
	}
}
function checkHtmlBackgroundMaterial() {
	setCurrentWindowStyle(getComputedStyle(document.documentElement).getPropertyValue(SpatialGlobalCustomVars.backgroundMaterial) || "none");
}
...
function checkCornerRadius() {
	setCornerRadius(parseCornerRadius$1(getComputedStyle(document.documentElement)));
}
function setCornerRadius(cornerRadius) {
	if (htmlCornerRadius.topLeading !== cornerRadius.topLeading || htmlCornerRadius.bottomLeading !== cornerRadius.bottomLeading || htmlCornerRadius.topTrailing !== cornerRadius.topTrailing || htmlCornerRadius.bottomTrailing !== cornerRadius.bottomTrailing) {
		session?.getSpatialScene()?.updateSpatialProperties({ cornerRadius });
...
function setOpacity(opacity) {
	session?.getSpatialScene().updateSpatialProperties({ opacity });
}
function checkOpacity() {
	const computedStyle = getComputedStyle(document.documentElement);
	setOpacity(parseFloat(computedStyle.getPropertyValue("opacity")));
}
function hijackDocumentElementStyle() {
	...
	const styleProxy = new Proxy(rawDocumentStyle, {
		set: function(target, key, value) {
			...
			if (key === SpatialGlobalCustomVars.backgroundMaterial) setCurrentWindowStyle(value);
			if (key === "border-radius" || key === "borderRadius" || key === "border-top-left-radius" || key === "borderTopLeftRadius" || key === "border-top-right-radius" || key === "borderTopRightRadius" || key === "border-bottom-left-radius" || key === "borderBottomLeftRadius" || key === "border-bottom-right-radius" || key === "borderBottomRightRadius") checkCornerRadius();
			if (key === "opacity") checkOpacity();
			return ret;
		},
		get: function(target, prop) {
			if (typeof target[prop] === "function") return function(...args) {
				if (prop === "setProperty") {
					...
					if (property === SpatialGlobalCustomVars.backgroundMaterial) setCurrentWindowStyle(value);
				} else if (prop === "removeProperty") {
					const [property] = args;
					if (property === SpatialGlobalCustomVars.backgroundMaterial) setCurrentWindowStyle("none");
				}
	...
	Object.defineProperty(document.documentElement, "style", { get: function() {
		return styleProxy;
	} });
}
function monitorExternalStyleChange() {
	new MutationObserver(checkCSSProperties).observe(document.head, {
		childList: true,
		subtree: true
	});
}
...
function monitorHTMLAttributeChange() {
	new MutationObserver((mutations) => {
		mutations.forEach((mutation) => {
			if (mutation.type === "attributes" && mutation.attributeName) checkCSSProperties();
		});
...
async function spatialWindowPolyfill() {
	if (!spatial$1.runInSpatialWeb()) return;
	session = await spatial$1.requestSession();
	if (document.readyState === "complete") checkCSSProperties();
...
//#endregion
//#region node_modules/.pnpm/@webspatial+react-sdk@1.6.1_@webspatial+core-sdk@1.6.1_react-dom@19.2.6_react@19.2.6__react@19.2.6/node_modules/@webspatial/react-sdk/dist/default/index.js
(function() {
	if (typeof window === "undefined") return;
	if (!window.__webspatialsdk__) window.__webspatialsdk__ = {};
...
function getInheritedStyleProps(computedStyle) {
	...
	var props = {};
	for (var cssName of propNames) if (computedStyle[cssName]) props[cssName] = computedStyle[cssName];
	return props;
}
function parseTransformOrigin(computedStyle) {
	const [x, y] = computedStyle.getPropertyValue("transform-origin").split(" ").map(parseFloat);
	const width = parseFloat(computedStyle.getPropertyValue("width"));
	const height = parseFloat(computedStyle.getPropertyValue("height"));
	return {
		x: width > 0 ? x / width : .5,
		y: height > 0 ? y / height : .5,
		z: .5
	};
}
...
function extractAndRemoveCustomProperties(cssText, properties) {
	...
	return {
		extractedValues,
		filteredCssText: cssText.split(";").filter((rule) => {
...
function joinToCSSText(cssKV) {
	return Object.entries(cssKV).map(([key, value]) => `${key}: ${value}`).join(";");
}
var SpatialContainerRefProxy = class {
	...
	*/
	...
	setMirrorClassNotify(fn) {
		...
		if (fn && this.standardRawDom) this.flushSyncTransformClassFromStandard(true);
	}
	...
	*/
	scheduleSyncTransformClassFromStandard() {
		if (this.classSyncMicrotaskQueued) return;
	...
	*/
	flushSyncTransformClassFromStandard(force) {
		...
		if (!force && probe && probe.className === name && this.lastMirroredClassName === name) return;
		this.lastMirroredClassName = name;
		if (probe) probe.className = name;
	...
	updateStandardSpatializedContainerDom(dom) {
		...
		const domProxy = new Proxy(dom, {
			get(target, prop) {
				...
				if (prop === "xrClientDepth") return target.style.getPropertyValue(SpatialCustomStyleVars.depth);
				if (prop === "xrOffsetBack") return target.style.getPropertyValue(SpatialCustomStyleVars.back);
				if (prop === "style") {
					if (!self.styleProxy) self.styleProxy = new Proxy(target.style, {
						get(target2, prop2) {
							if (prop2 === "visibility" || prop2 === "transform") return self.transformVisibilityTaskContainerDom?.style.getPropertyValue(prop2);
							const value2 = Reflect.get(target2, prop2);
							if (typeof value2 === "function") if (prop2 === "setProperty" || prop2 === "removeProperty" || prop2 === "getPropertyValue") return function(...args) {
								const validProperties = ["visibility", "transform"];
								const [property] = args;
								if (validProperties.includes(property)) {
									if (prop2 === "setProperty") {
										const [, kValue] = args;
										self.transformVisibilityTaskContainerDom?.style.setProperty(property, kValue);
									} else if (prop2 === "removeProperty") self.transformVisibilityTaskContainerDom?.style.removeProperty(property);
									else if (prop2 === "getPropertyValue") return self.transformVisibilityTaskContainerDom?.style.getPropertyValue(property);
								} else return value2.apply(this, args);
							}.bind(target2);
							else return value2.bind(target2);
							else return value2;
						},
						set(target2, prop2, value2) {
							if (prop2 === "visibility") {
								self.transformVisibilityTaskContainerDom?.style.setProperty("visibility", value2);
								return true;
							}
							if (prop2 === "transform") {
								self.transformVisibilityTaskContainerDom?.style.setProperty("transform", value2);
								return true;
							}
							if (prop2 === SpatialCustomStyleVars.backgroundMaterial) target2.setProperty(SpatialCustomStyleVars.backgroundMaterial, value2);
							else if (prop2 === SpatialCustomStyleVars.back) target2.setProperty(SpatialCustomStyleVars.back, value2);
							else if (prop2 === SpatialCustomStyleVars.xrZIndex) target2.setProperty(SpatialCustomStyleVars.xrZIndex, value2);
							else if (prop2 === SpatialCustomStyleVars.depth) target2.setProperty(SpatialCustomStyleVars.depth, value2);
							else if (prop2 === "cssText") {
								const toFilteredCSSProperties = ["transform", "visibility"];
								const { extractedValues, filteredCssText } = extractAndRemoveCustomProperties(value2, toFilteredCSSProperties);
								toFilteredCSSProperties.forEach((key) => {
									if (extractedValues[key]) self.transformVisibilityTaskContainerDom?.style.setProperty(key, extractedValues[key]);
									else target2.removeProperty(key);
								});
								const appendedCSSText = joinToCSSText({
									transform: "none",
									visibility: "hidden"
								});
								return Reflect.set(target2, prop2, [appendedCSSText, filteredCssText].join(";"));
							}
				...
				if (typeof prop === "string" && self.extraRefProps) {
					if (!cacheExtraRefProps) cacheExtraRefProps = self.extraRefProps(domProxy);
					const extraProps = cacheExtraRefProps;
					if (extraProps.hasOwnProperty(prop)) return extraProps[prop];
				}
				const value = Reflect.get(target, prop);
				if (typeof value === "function") {
					if ("removeAttribute" === prop) return function(...args) {
						const [property] = args;
						if (property === "style") {
			...
			set(target, prop, value) {
				if (prop === "className") {
					if (value && String(value).indexOf("xr-spatial-default") === -1) value = value + " xr-spatial-default";
				}
				if (typeof prop === "string" && self.extraRefProps) {
					if (!cacheExtraRefProps) cacheExtraRefProps = self.extraRefProps(domProxy);
				...
				if (ok && prop === "className") self.scheduleSyncTransformClassFromStandard();
				return ok;
			}
		});
		...
		this.styleProxy = void 0;
		this.updateDomProxyToRef();
		Object.assign(dom, { __targetProxy: domProxy });
	...
	updateTransformVisibilityTaskContainerDom(dom) {
		...
		if (!dom) this.lastMirroredClassName = null;
		this.scheduleSyncTransformClassFromStandard();
		this.updateDomProxyToRef();
	}
	updateDomProxyToRef() {
		const ref = this.ref;
		if (!ref) return;
		if (this.domProxy && this.transformVisibilityTaskContainerDom) if (typeof ref === "function") ref(this.domProxy);
		else ref.current = this.domProxy;
...
function hijackGetComputedStyle() {
	...
	window.getComputedStyle = (element, pseudoElt) => {
		...
		if (dom) return rawFn(dom, pseudoElt);
		return rawFn(element, pseudoElt);
	};
}
function useDomProxy(ref, extraRefProps) {
	const spatialContainerRefProxy = (0, import_react.useRef)(new SpatialContainerRefProxy(ref, extraRefProps));
	(0, import_react.useEffect)(() => {
		spatialContainerRefProxy.current.updateRef(ref);
	}, [ref]);
	return {
		transformVisibilityTaskContainerCallback: (0, import_react.useCallback)((el) => {
			spatialContainerRefProxy.current.updateTransformVisibilityTaskContainerDom(el);
		}, []),
		standardSpatializedContainerCallback: (0, import_react.useCallback)((el) => {
...
var SpatializedContainerObject = class {
	...
	updateSpatialTransformVisibility(spatialId, spatialTransformVisibility) {
		...
		this.fnsForSpatialTransformVisibility[spatialId]?.forEach((fn) => fn(spatialTransformVisibility));
	}
	...
	onSpatialTransformVisibilityChange(spatialId, fn) {
		if (!this.fnsForSpatialTransformVisibility[spatialId]) this.fnsForSpatialTransformVisibility[spatialId] = [];
		this.fnsForSpatialTransformVisibility[spatialId].push(fn);
		if (this.spatialId2transformVisibility[spatialId]) fn(this.spatialId2transformVisibility[spatialId]);
	}
	offSpatialTransformVisibilityChange(spatialId, fn) {
		const fns = this.fnsForSpatialTransformVisibility[spatialId];
		if (fns) this.fnsForSpatialTransformVisibility[spatialId] = fns.filter((f) => f !== fn);
	}
	...
	querySpatialDomBySpatialId(spatialId) {
		if (this.domSpatialId === spatialId) return this.dom;
		if (!this.dom) return null;
		if (!this.spatialId2dom[spatialId]) {
			const spatialDom = this.dom.querySelector(`[${SpatialID}="${spatialId}"]`);
			if (spatialDom) this.spatialId2dom[spatialId] = spatialDom;
		}
		return this.spatialId2dom[spatialId];
	}
	queryParentSpatialDomBySpatialId(spatialId) {
		if (this.domSpatialId === spatialId) return null;
		if (this.spatialId2parentSpatialDom[spatialId]) return this.spatialId2parentSpatialDom[spatialId];
		...
		if (spatialDom) {
			...
			while (parentSpatialDom && spatialDom !== this.dom) if (parentSpatialDom.hasAttribute(SpatialID)) break;
			else parentSpatialDom = parentSpatialDom.parentElement;
	...
	getSpatialId(layer, isInStandardInstance, name = "") {
		if (this.layerSequences[layer] === void 0) this.layerSequences[layer] = [0, 0];
		...
		return `${name}_${layer}_${sequenceId}`;
	}
};
var SpatializedContainerContext = (0, import_react.createContext)(null);
function use2DFrameDetector(ref) {
	const spatializedContainerObject = (0, import_react.useContext)(SpatializedContainerContext);
	const notify2DFrameChange = (0, import_react.useCallback)(() => {
		ref.current && spatializedContainerObject.notify2DFramePlaceHolderChange(ref.current);
	}, [ref.current, spatializedContainerObject]);
	(0, import_react.useLayoutEffect)(notify2DFrameChange, [notify2DFrameChange]);
	(0, import_react.useEffect)(() => {
		if (!ref.current || !spatializedContainerObject) {
			console.warn("Ref is not attached to the DOM or spatializedContainerObject is not available");
			return;
		}
...
function useSpatialTransformVisibilityWatcher(spatialId) {
	const [transformExist, setTransformExist] = (0, import_react.useState)(false);
	const spatializedContainerObject = (0, import_react.useContext)(SpatializedContainerContext);
	(0, import_react.useEffect)(() => {
		...
		return () => {
			spatializedContainerObject.offSpatialTransformVisibilityChange(spatialId, fn);
		};
	}, [spatialId, spatializedContainerObject]);
	return transformExist;
}
function useInternalRef(ref) {
	const refInternal = (0, import_react.useRef)(null);
	return {
		refInternal,
		refInternalCallback: (0, import_react.useCallback)((node) => {
...
function StandardSpatializedContainerBase(props, ref) {
	const { component: Component, style: inStyle = {}, className, inStandardSpatializedContainer = false, ...restProps } = props;
	const { refInternal, refInternalCallback } = useInternalRef(ref);
	if (!inStandardSpatializedContainer) use2DFrameDetector(refInternal);
	const extraStyle = {
		visibility: "hidden",
		transition: "none",
		transform: useSpatialTransformVisibilityWatcher(props[SpatialID]) ? "translateZ(0)" : "none"
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Component, {
		...
		className: className ? `${className} xr-spatial-default` : "xr-spatial-default",
		...restProps
	});
}
var StandardSpatializedContainer = (0, import_react.forwardRef)(StandardSpatializedContainerBase);
...
function notifyUpdateStandInstanceLayout() {
	document.dispatchEvent(new CustomEvent("standInstanceLayout", { detail: {} }));
}
function notifyDOMUpdate(mutationsList) {
	document.dispatchEvent(new CustomEvent("domUpdated", { detail: mutationsList }));
}
...
function useSpatialTransformVisibility(spatialId, ref) {
	const spatializedContainerObject = (0, import_react.useContext)(SpatializedContainerContext);
	const checkSpatialStyleUpdate = (0, import_react.useCallback)(() => {
		if (!ref.current) return;
		const spatialTransformVisibility = parseTransformAndVisibilityProperties(ref.current);
		spatializedContainerObject.updateSpatialTransformVisibility(spatialId, spatialTransformVisibility);
	}, []);
	(0, import_react.useEffect)(() => {
	...
	(0, import_react.useEffect)(() => {
		...
		document.addEventListener("domUpdated", onDomUpdated);
		return () => {
			document.removeEventListener("domUpdated", onDomUpdated);
		};
	}, []);
}
...
function createOrGetCSSParserDivContainer() {
	if (cssParserDivContainer && !cssParserDivContainer.parentElement) document?.body.appendChild(cssParserDivContainer);
...
function TransformVisibilityTaskContainerBase(props, ref) {
	...
	if (!cssParserDivContainer2) return null;
...
var TransformVisibilityTaskContainer = (0, import_react.forwardRef)(TransformVisibilityTaskContainerBase);
var spatial = null;
var _currentSession = null;
function getSession() {
	if (isSSREnv()) return null;
	if (!spatial) spatial = new Spatial();
	if (!spatial.isSupported()) return null;
	if (_currentSession) return _currentSession;
	_currentSession = spatial.requestSession();
	return _currentSession;
}
var SpatialLayerContext = (0, import_react.createContext)(0);
var PortalInstanceObject = class {
	...
	constructor(spatialId, spatializedContainerObject, parentPortalInstanceObject, getExtraSpatializedElementProperties2) {
		...
		this.spatializedElementPromise = new Promise((resolve) => {
			this.spatializedElementResolver = resolve;
		});
	}
	init() {
		this.spatializedContainerObject.onSpatialTransformVisibilityChange(this.spatialId, this.onSpatialTransformVisibilityChange);
	}
	destroy() {
		this.spatializedContainerObject.offSpatialTransformVisibilityChange(this.spatialId, this.onSpatialTransformVisibilityChange);
	}
	...
	notify2DFrameChange() {
		const dom = this.spatializedContainerObject.querySpatialDomBySpatialId(this.spatialId);
		if (!dom) return;
		...
		Object.assign(dom, { __innerSpatializedElement });
	}
	...
	async addToParent(spatializedElement) {
		if (this.inAddingToParent) return;
		this.inAddingToParent = true;
		if (this.isFixedPosition || !this.parentPortalInstanceObject) await (await getSession().getSpatialScene()).addSpatializedElement(spatializedElement);
		else (await this.parentPortalInstanceObject.getSpatializedElement()).addSpatializedElement(spatializedElement);
	...
	updateSpatializedElementProperties() {
		...
		if (!dom || !spatializedElement || !visibility || !this.transformMatrix) return;
		const computedStyle = this.computedStyle;
		const isFixedPosition = this.isFixedPosition;
		let domRect = dom.getBoundingClientRect();
		let { x, y } = domRect;
		if (!isFixedPosition) {
			const parentDom = this.spatializedContainerObject.queryParentSpatialDomBySpatialId(this.spatialId);
			if (parentDom) {
		...
		const zIndex = parseFloat(computedStyle.getPropertyValue(SpatialCustomStyleVars.xrZIndex)) || 0;
		const backOffset = parseFloat(computedStyle.getPropertyValue(SpatialCustomStyleVars.back)) || 0;
		const depth = parseFloat(computedStyle.getPropertyValue(SpatialCustomStyleVars.depth)) || 0;
		...
		Object.assign(this.dom, { __spatializedElement: spatializedElement });
	}
};
var PortalInstanceContext = (0, import_react.createContext)(null);
...
function useSpatializedElement(createSpatializedElement2, portalInstanceObject) {
	const [spatializedElement, setSpatializedElement] = (0, import_react.useState)();
	const elementRef = (0, import_react.useRef)(void 0);
	(0, import_react.useEffect)(() => {
		let isDestroyed = false;
		createSpatializedElement2().then((inSpatializedElement) => {
			...
			} else inSpatializedElement?.destroy();
		});
		return () => {
			isDestroyed = true;
...
function renderPlaceholderInSubPortal(portalInstanceObject, El) {
	...
	if (!(inPortalInstanceEnv && portalInstanceObject && portalInstanceObject.domRect && position !== "absolute" && position !== "fixed")) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, {});
	const { width, height } = portalInstanceObject.domRect;
	const display = portalInstanceObject.computedStyle.getPropertyValue("display");
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(El, {
		[SpatialID]: spatialId,
		style: {
			position: "relative",
			width: `${width}px`,
...
function PortalSpatializedContainer(props) {
	const { spatializedContent: Content, createSpatializedElement: createSpatializedElement2, getExtraSpatializedElementProperties: getExtraSpatializedElementProperties2, onSpatialTap, onSpatialDragStart, onSpatialDrag, onSpatialDragEnd, onSpatialRotate, onSpatialRotateEnd, onSpatialMagnify, onSpatialMagnifyEnd, spatialEventOptions, [SpatialID]: spatialId, ...restProps } = props;
	const spatializedContainerObject = (0, import_react.useContext)(SpatializedContainerContext);
	const parentPortalInstanceObject = (0, import_react.useContext)(PortalInstanceContext);
	const portalInstanceObject = (0, import_react.useMemo)(() => new PortalInstanceObject(spatialId, spatializedContainerObject, parentPortalInstanceObject, getExtraSpatializedElementProperties2), []);
	(0, import_react.useEffect)(() => {
	...
	const spatializedElement = useSpatializedElement(createSpatializedElement2, portalInstanceObject);
	const PlaceholderEl = renderPlaceholderInSubPortal(portalInstanceObject, props.component);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialTap = onSpatialTap;
	}, [spatializedElement, onSpatialTap]);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialDrag = onSpatialDrag;
	}, [spatializedElement, onSpatialDrag]);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialDragEnd = onSpatialDragEnd;
	}, [spatializedElement, onSpatialDragEnd]);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialRotate = onSpatialRotate;
	}, [spatializedElement, onSpatialRotate]);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialRotateEnd = onSpatialRotateEnd;
	}, [spatializedElement, onSpatialRotateEnd]);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialMagnify = onSpatialMagnify;
	}, [spatializedElement, onSpatialMagnify]);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialMagnifyEnd = onSpatialMagnifyEnd;
	}, [spatializedElement, onSpatialMagnifyEnd]);
	(0, import_react.useEffect)(() => {
		if (spatializedElement) spatializedElement.onSpatialDragStart = onSpatialDragStart;
	...
	(0, import_react.useEffect)(() => {
		...
		spatializedElement.updateProperties({ rotateConstrainedToAxis: axis });
	}, [spatializedElement, constrainedAxisKey(spatialEventOptions?.constrainedToAxis)]);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(PortalInstanceContext.Provider, {
		value: portalInstanceObject,
		children: [spatializedElement && portalInstanceObject.dom && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Content, {
			spatializedElement,
			...restProps
		}), PlaceholderEl]
	});
}
var InsideAttachmentContext = (0, import_react.createContext)(false);
var useInsideAttachment = () => (0, import_react.useContext)(InsideAttachmentContext);
function createEventProxy(event, currentTargetGetter, offsetXGetter, offsetYGetter, offsetZGetter, clientXGetter, clientYGetter, clientZGetter, translationXGetter, translationYGetter, translationZGetter, quaternionGetter, magnificationGetter) {
	return new Proxy(event, { get(target, prop) {
		if (prop === "currentTarget") return currentTargetGetter();
		if (prop === "isTrusted") return true;
		if (prop === "bubbles") return false;
		if (prop === "offsetX" && offsetXGetter) return offsetXGetter(target) ?? 0;
		if (prop === "offsetY" && offsetYGetter) return offsetYGetter(target) ?? 0;
		if (prop === "offsetZ" && offsetZGetter) return offsetZGetter(target) ?? 0;
		if (prop === "clientX" && clientXGetter) return clientXGetter(target) ?? 0;
		if (prop === "clientY" && clientYGetter) return clientYGetter(target) ?? 0;
		if (prop === "clientZ" && clientZGetter) return clientZGetter(target) ?? 0;
		if (prop === "translationX" && translationXGetter) return translationXGetter(target) ?? 0;
		if (prop === "translationY" && translationYGetter) return translationYGetter(target) ?? 0;
		if (prop === "translationZ" && translationZGetter) return translationZGetter(target) ?? 0;
		if (prop === "quaternion" && quaternionGetter) return quaternionGetter(target) ?? {
			x: 0,
			y: 0,
			z: 0,
			w: 1
		};
		if (prop === "magnification" && magnificationGetter) return magnificationGetter(target) ?? 1;
		return Reflect.get(target, prop);
	} });
}
function createEventHandler(handler, currentTargetGetter, offsetXGetter, offsetYGetter, offsetZGetter, clientXGetter, clientYGetter, clientZGetter, translationXGetter, translationYGetter, translationZGetter, quaternionGetter, magnificationGetter) {
	return handler ? (event) => {
		handler(createEventProxy(event, currentTargetGetter, offsetXGetter, offsetYGetter, offsetZGetter, clientXGetter, clientYGetter, clientZGetter, translationXGetter, translationYGetter, translationZGetter, quaternionGetter, magnificationGetter));
	} : void 0;
}
function useSpatialEventsBase(spatialEvents, currentTargetGetter) {
	const onSpatialTap = createEventHandler(spatialEvents.onSpatialTap, currentTargetGetter, (ev) => ev.detail?.location3D?.x, (ev) => ev.detail?.location3D?.y, (ev) => ev.detail?.location3D?.z, (ev) => ev.detail?.globalLocation3D?.x, (ev) => ev.detail?.globalLocation3D?.y, (ev) => ev.detail?.globalLocation3D?.z);
	const onSpatialDrag = createEventHandler(spatialEvents.onSpatialDrag, currentTargetGetter, void 0, void 0, void 0, void 0, void 0, void 0, (ev) => ev.detail?.translation3D?.x, (ev) => ev.detail?.translation3D?.y, (ev) => ev.detail?.translation3D?.z);
	const onSpatialDragEnd = createEventHandler(spatialEvents.onSpatialDragEnd, currentTargetGetter);
	const onSpatialRotate = createEventHandler(spatialEvents.onSpatialRotate, currentTargetGetter, void 0, void 0, void 0, void 0, void 0, void 0, void 0, void 0, void 0, (ev) => ev.detail?.quaternion);
	const onSpatialRotateEnd = createEventHandler(spatialEvents.onSpatialRotateEnd, currentTargetGetter);
	const onSpatialMagnify = createEventHandler(spatialEvents.onSpatialMagnify, currentTargetGetter, void 0, void 0, void 0, void 0, void 0, void 0, void 0, void 0, void 0, void 0, (ev) => ev.detail?.magnification);
	const onSpatialMagnifyEnd = createEventHandler(spatialEvents.onSpatialMagnifyEnd, currentTargetGetter);
	return {
		onSpatialTap,
		onSpatialDragStart: createEventHandler(spatialEvents.onSpatialDragStart, currentTargetGetter, (ev) => ev.detail?.startLocation3D?.x, (ev) => ev.detail?.startLocation3D?.y, (ev) => ev.detail?.startLocation3D?.z, (ev) => ev.detail?.globalLocation3D?.x, (ev) => ev.detail?.globalLocation3D?.y, (ev) => ev.detail?.globalLocation3D?.z),
		onSpatialDrag,
		onSpatialDragEnd,
		onSpatialRotate,
		onSpatialRotateEnd,
...
function useSpatialEvents(spatialEvents, spatialContainerRefProxy) {
	return useSpatialEventsBase(spatialEvents, () => spatialContainerRefProxy.current?.domProxy);
}
function useSpatialEventsWhenSpatializedContainerExist(spatialEvents, spatialId, spatializedContainerObject) {
	return useSpatialEventsBase(spatialEvents, () => {
		return spatializedContainerObject.getSpatialContainerRefProxyBySpatialId(spatialId)?.domProxy;
	});
}
var SSRContext = (0, import_react.createContext)(false);
function useSSRPhase() {
	const isSSRContext = (0, import_react.useContext)(SSRContext);
	const isServer = typeof window === "undefined";
	const [hydrated, setHydrated] = (0, import_react.useState)(false);
	(0, import_react.useEffect)(() => setHydrated(true), []);
	if (isServer) return "ssr";
	if (isSSRContext) return hydrated ? "after-hydrate" : "hydrate";
	else return "after-hydrate";
}
function withSSRSupported(Component) {
	const ClientOnlyComponent = (props, ref) => {
		const phase = useSSRPhase();
		let renderType = "real";
		if (phase === "ssr" || phase === "hydrate") renderType = "fake";
		if (renderType === "fake") {
...
function SpatializedContainerBase(inprops, ref) {
	...
	if (!isWebSpatialEnv || insideAttachment) {
		if (insideAttachment) console.warn(`[WebSpatial] ${inprops.component || "Spatial element"} cannot be used inside AttachmentAsset. Rendering as plain HTML.`);
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DegradedContainer, {
			...inprops,
			innerRef: ref
		});
	}
	const layer = (0, import_react.useContext)(SpatialLayerContext) + 1;
	const rootSpatializedContainerObject = (0, import_react.useContext)(SpatializedContainerContext);
	const inSpatializedContainer = !!rootSpatializedContainerObject;
	const inPortalInstanceEnv = !!(0, import_react.useContext)(PortalInstanceContext);
	const isInStandardInstance = !inPortalInstanceEnv;
	const spatialId = (0, import_react.useMemo)(() => {
	...
	const { onSpatialTap, onSpatialDragStart, onSpatialDrag, onSpatialDragEnd, onSpatialRotate, onSpatialRotateEnd, onSpatialMagnify, onSpatialMagnifyEnd, extraRefProps, ...props } = inprops;
	if (inSpatializedContainer) if (inPortalInstanceEnv) {
		const spatialEvents = useSpatialEventsWhenSpatializedContainerExist({
			...
			onSpatialMagnify,
			onSpatialMagnifyEnd
		}, spatialId, rootSpatializedContainerObject);
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpatialLayerContext.Provider, {
			value: layer,
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PortalSpatializedContainer, {
				...spatialIdProps,
				...props,
				...spatialEvents
			})
		...
		const { transformVisibilityTaskContainerCallback, standardSpatializedContainerCallback, spatialContainerRefProxy } = useDomProxy(ref, extraRefProps);
		const [probeClassName, setProbeClassName] = (0, import_react.useState)(() => props.className ?? "");
		const notifyProbeClass = (0, import_react.useCallback)((name) => {
		...
		(0, import_react.useEffect)(() => {
			rootSpatializedContainerObject.updateSpatialContainerRefProxyInfo(spatialId, spatialContainerRefProxy.current);
		}, [spatialContainerRefProxy.current]);
		const { spatializedContent, createSpatializedElement: createSpatializedElement2, getExtraSpatializedElementProperties: getExtraSpatializedElementProperties2, spatialEventOptions: _nestedSpatialEventOptions, ...restProps } = props;
		return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(SpatialLayerContext.Provider, {
			value: layer,
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(StandardSpatializedContainer, {
				ref: standardSpatializedContainerCallback,
				...spatialIdProps,
				...restProps,
				inStandardSpatializedContainer: true
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TransformVisibilityTaskContainer, {
		...
		const spatializedContainerObject = (0, import_react.useMemo)(() => new SpatializedContainerObject(), []);
		const { spatializedContent, createSpatializedElement: createSpatializedElement2, getExtraSpatializedElementProperties: getExtraSpatializedElementProperties2, spatialEventOptions: _rootSpatialEventOptions, ...restProps } = props;
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpatialLayerContext.Provider, {
			value: layer,
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(SpatializedContainerContext.Provider, {
				value: spatializedContainerObject,
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(StandardSpatializedContainer, {
						ref: standardSpatializedContainerCallback,
						...spatialIdProps,
						...restProps,
						inStandardSpatializedContainer: false
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PortalSpatializedContainer, {
						...spatialIdProps,
						...props,
						...spatialEvents
					}),
...
var SpatializedContainer = withSSRSupported((0, import_react.forwardRef)(SpatializedContainerBase));
function asyncLoadStyleToChildWindow(childWindow, link, isCurrent) {
	return new Promise((resolve) => {
		const { href } = link;
		link.href = `${href}${href.includes("?") ? "&" : "?"}uniqueURL=${Math.random()}`;
		let finished = false;
		const finish = (ok) => {
			if (finished) return;
			finished = true;
...
async function syncParentHeadToChild(childWindow) {
	...
	const parentStylesheets = Array.from(document.head.querySelectorAll("link[rel=\"stylesheet\"][href]"));
	const desiredStylesheetKeys = /* @__PURE__ */ new Set();
	for (const link of parentStylesheets) if (link.href) desiredStylesheetKeys.add(link.href);
	const existingSyncedLinks = Array.from(head.querySelectorAll(`link[rel="stylesheet"][${WEBSPATIAL_SYNC_ATTR}="1"]`));
	...
	head.querySelectorAll(`style[${WEBSPATIAL_SYNC_ATTR}="1"]`).forEach((n) => n.parentNode?.removeChild(n));
	...
	const currentSyncedLinks = Array.from(head.querySelectorAll(`link[rel="stylesheet"][${WEBSPATIAL_SYNC_ATTR}="1"]`));
	for (const link of currentSyncedLinks) currentKeys.add(link.getAttribute(WEBSPATIAL_SYNC_KEY_ATTR) ?? link.href);
	for (const link of parentStylesheets) {
		...
		styleLoadedPromises.push(asyncLoadStyleToChildWindow(childWindow, node, isCurrent));
	}
...
function useSyncDocumentTitle(windowProxy, spatializedElement, name) {
	(0, import_react.useEffect)(() => {
		...
		spatializedElement.updateProperties({ name });
	}, [name]);
}
function SpatializedContent(props) {
	...
	useSyncHeadStyles(windowProxy, { subtree: false });
	useSyncDocumentTitle(windowProxy, spatialized2DElement, restProps["data-name"] || "");
	return (0, import_react_dom.createPortal)(getJSXPortalInstance(restProps, (0, import_react.useContext)(PortalInstanceContext)), windowProxy.document.body);
}
function getExtraSpatializedElementProperties(computedStyle) {
	...
	const material = computedStyle.getPropertyValue(SpatialCustomStyleVars.backgroundMaterial);
	const properties = {};
...
async function createSpatializedElement() {
	...
	if (viewport) viewport?.setAttribute("content", " initial-scale=1.0, maximum-scale=1.0, user-scalable=no");
	else {
...
function Spatialized2DElementContainerBase(props, ref) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpatializedContainer, {
		ref,
...
var Spatialized2DElementContainer = (0, import_react.forwardRef)(Spatialized2DElementContainerBase);
...
function createLoadEvent(type, targetGetter) {
	...
	return new Proxy(event, { get(target, prop) {
		if (prop === "target") return targetGetter();
		return Reflect.get(target, prop);
	} });
}
...
function collectSources(children) {
	...
	import_react.Children.forEach(children, (child) => {
		if ((0, import_react.isValidElement)(child) && child.type === "source") {
...
function SpatializedContent2(props) {
	...
	const portalInstanceObject = (0, import_react.useContext)(PortalInstanceContext);
	const modelURL = (0, import_react.useMemo)(() => getAbsoluteURL(src), [src]);
	const sources = (0, import_react.useMemo)(() => collectSources(children), [children]);
	(0, import_react.useEffect)(() => {
		spatializedElement.updateProperties({
			modelURL: modelURL ?? "",
			sources,
	...
	(0, import_react.useEffect)(() => {
		if (onLoad) spatializedElement.onLoadCallback = () => {
			onLoad(createLoadSuccessEvent(() => portalInstanceObject.dom.__targetProxy));
		};
		else spatializedElement.onLoadCallback = void 0;
	}, [onLoad]);
	(0, import_react.useEffect)(() => {
		if (onError) spatializedElement.onLoadFailureCallback = () => {
			onError(createLoadFailureEvent(() => portalInstanceObject.dom.__targetProxy));
		};
		else spatializedElement.onLoadFailureCallback = void 0;
	}, [onError]);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, {});
}
function SpatializedStatic3DElementContainerBase(props, ref) {
	const promiseRef = (0, import_react.useRef)(null);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpatializedContainer, {
		ref,
		component: "div",
		createSpatializedElement: (0, import_react.useCallback)(() => {
			promiseRef.current = getSession().createSpatializedStatic3DElement(getAbsoluteURL(props.src), collectSources(props.children));
			return promiseRef.current;
		}, []),
		spatializedContent: SpatializedContent2,
		extraRefProps: (0, import_react.useCallback)((domProxy) => {
			let modelTransform = new DOMMatrixReadOnly();
			return {
				get currentSrc() {
					return domProxy.__spatializedElement?.currentSrc ?? "";
				},
				get ready() {
				...
				set entityTransform(value) {
					...
					domProxy.__spatializedElement?.updateModelTransform(modelTransform);
				},
				async play() {
					await domProxy.__spatializedElement?.play();
				},
				async pause() {
					await domProxy.__spatializedElement?.pause();
				},
				get paused() {
					return domProxy.__spatializedElement?.paused ?? true;
				},
				get duration() {
					return domProxy.__spatializedElement?.duration ?? 0;
				},
				get playbackRate() {
					return domProxy.__spatializedElement?.playbackRate ?? 1;
				},
				set playbackRate(value) {
					const spatializedElement = domProxy.__spatializedElement;
					if (spatializedElement) spatializedElement.playbackRate = value;
				}
			};
		}, []),
		...props
...
var SpatializedStatic3DElementContainer = (0, import_react.forwardRef)(SpatializedStatic3DElementContainerBase);
var CachedSpatialized2DElementContainerType = /* @__PURE__ */ new Map();
function withSpatialized2DElementContainer(Component) {
	if (CachedSpatialized2DElementContainerType.has(Component)) return CachedSpatialized2DElementContainerType.get(Component);
	else {
		const TypedSpatialized2DElementContainer = (0, import_react.forwardRef)((givenProps, ref) => {
			const { component: ignoreComponent, ...props } = givenProps;
			return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Spatialized2DElementContainer, {
				component: Component,
				...props,
				ref
			});
		});
		CachedSpatialized2DElementContainerType.set(Component, TypedSpatialized2DElementContainer);
		CachedSpatialized2DElementContainerType.set(TypedSpatialized2DElementContainer, TypedSpatialized2DElementContainer);
...
function useMonitorDomChange(inRef) {
	...
	return (0, import_react.useMemo)(() => new Proxy(ref, { set: function(target, key, value) {
		if (key === "current") {
			if (inRef) {
				if (typeof inRef === "function") inRef(value);
				else if (inRef) inRef.current = value;
			}
		}
...
var SpatialMonitor = (0, import_react.forwardRef)(SpatialMonitorBase);
var cachedWithSpatialMonitorType = /* @__PURE__ */ new Map();
function withSpatialMonitor(El) {
	if (cachedWithSpatialMonitorType.has(El)) return cachedWithSpatialMonitorType.get(El);
	else {
		const WithSpatialMonitorComponent = (0, import_react.forwardRef)((givenProps, givenRef) => {
			const { El: _, ...props } = givenProps;
			return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpatialMonitor, {
				El,
				...props,
				ref: givenRef
			});
		...
		cachedWithSpatialMonitorType.set(cachedWithSpatialMonitorType, cachedWithSpatialMonitorType);
		return WithSpatialMonitorComponent;
	}
}
var RealityContext = (0, import_react.createContext)(null);
var useRealityContext = () => (0, import_react.useContext)(RealityContext);
var ParentContext = (0, import_react.createContext)(null);
var useParentContext = () => (0, import_react.useContext)(ParentContext);
var AttachmentRegistry = class {
	containers = /* @__PURE__ */ new Map();
	listeners = /* @__PURE__ */ new Map();
	addContainer(name, instanceId, container) {
		if (!this.containers.has(name)) this.containers.set(name, /* @__PURE__ */ new Map());
	...
	removeContainer(name, instanceId) {
		...
		if (this.containers.get(name)?.size === 0) this.containers.delete(name);
		this.notifyListeners(name);
	}
	getContainers(name) {
	...
	onContainersChange(name, cb) {
		...
		return () => {
			if (this.listeners.get(name) === cb) this.listeners.delete(name);
		};
	}
...
var ResourceRegistry = class {
	...
	removeAndDestroy(id) {
		...
		if (p) p.then((spatialObj) => spatialObj.destroy()).catch(() => {});
		this.resources.delete(id);
	}
	get(id) {
	...
	destroy() {
		...
		pending.forEach((promise) => promise.then((spatialObj) => spatialObj.destroy()).catch(() => {}));
	}
};
...
function useEntityTransform(entity, { position, rotation, scale }) {
	...
	(0, import_react.useEffect)(() => {
		if (!entity) return;
		if (!(!shallowEqualVec3(last.current.position, position) || !shallowEqualRotation(last.current.rotation, rotation) || !shallowEqualVec3(last.current.scale, scale))) return;
		last.current = {
			position,
			rotation,
			scale
...
var useEntityRef = (ref, instance) => {
	(0, import_react.useImperativeHandle)(ref, () => instance);
};
var EntityRef = class {
	_entity;
	_ctx;
	...
	async convertFromEntityToEntity(fromEntityId, toEntityId, position) {
		...
		try {
			...
			return (await this._entity.convertFromEntityToEntity(fromEnt.id, toEnt.id, position))?.data ?? position;
		} catch {
			return position;
		}
	}
	async convertFromEntityToReality(entityId, position) {
		...
		try {
			...
			return (await this._entity.convertFromEntityToScene(ent.id, position))?.data ?? position;
		} catch {
			return position;
		}
	}
	async convertFromRealityToEntity(entityId, position) {
		...
		try {
			const ent = await this._ctx?.resourceRegistry.get(entityId);
			if (!ent) return position;
			return (await this._entity.convertFromSceneToEntity(ent.id, position))?.data ?? position;
		} catch {
			return position;
		}
	}
...
function createEventProxy2(ev, instance) {
	return new Proxy(ev, { get(target, prop) {
		if (prop === "currentTarget") return instance;
		if (prop === "target") {
			const origin = target.__origin;
			if (origin) return new EntityRef(origin, null);
			return instance;
		}
		if (prop === "bubbles") return true;
		if (prop === "offsetX") {
			const type = target.type;
			if (type === "spatialtap") return target.detail?.location3D?.x ?? 0;
			if (type === "spatialdragstart") return target.detail?.startLocation3D?.x ?? 0;
			return;
		}
		if (prop === "offsetY") {
			const type = target.type;
			if (type === "spatialtap") return target.detail?.location3D?.y ?? 0;
			if (type === "spatialdragstart") return target.detail?.startLocation3D?.y ?? 0;
			return;
		}
		if (prop === "offsetZ") {
			const type = target.type;
			if (type === "spatialtap") return target.detail?.location3D?.z ?? 0;
			if (type === "spatialdragstart") return target.detail?.startLocation3D?.z ?? 0;
			return;
		}
		if (prop === "translationX") {
			if (target.type === "spatialdrag") return target.detail?.translation3D?.x ?? 0;
			return;
		}
		if (prop === "translationY") {
			if (target.type === "spatialdrag") return target.detail?.translation3D?.y ?? 0;
			return;
		}
		if (prop === "translationZ") {
			if (target.type === "spatialdrag") return target.detail?.translation3D?.z ?? 0;
			return;
		}
		if (prop === "quaternion") {
			if (target.type === "spatialrotate") return target.detail?.quaternion ?? {
				x: 0,
				y: 0,
				z: 0,
				w: 1
		...
		if (prop === "magnification") {
			if (target.type === "spatialmagnify") return target.detail?.magnification ?? 1;
			return;
		}
		if (prop === "clientX") {
			const type = target.type;
			if (type === "spatialtap" || type === "spatialdragstart") return target.detail?.globalLocation3D?.x ?? 0;
			return;
		}
		if (prop === "clientY") {
			const type = target.type;
			if (type === "spatialtap" || type === "spatialdragstart") return target.detail?.globalLocation3D?.y ?? 0;
			return;
		}
		if (prop === "clientZ") {
			const type = target.type;
			if (type === "spatialtap" || type === "spatialdragstart") return target.detail?.globalLocation3D?.z ?? 0;
			return;
		}
...
var useEntityEvent = ({ instance, ...handlers }) => {
	const eventsSetRef = (0, import_react.useRef)(/* @__PURE__ */ new Set());
	(0, import_react.useEffect)(() => {
	...
	(0, import_react.useEffect)(() => {
		...
		if (!entity) return;
		return () => {
			for (let x of eventsSetRef.current) entity.removeEvent(x);
...
var useRealityEvents = ({ instance, ...handlers }) => {
	...
	(0, import_react.useEffect)(() => {
		...
		return () => {
			if (instance) {
				for (let x of eventsSetRef.current) instance.removeEvent(x);
				eventsSetRef.current.clear();
			}
		};
...
var useEntity = ({ ref, id, position, rotation, scale, enableInput, onSpatialTap, onSpatialDragStart, onSpatialDrag, onSpatialDragEnd, onSpatialRotate, onSpatialRotateEnd, onSpatialMagnify, onSpatialMagnifyEnd, createEntity, recreateKey }) => {
	const ctx = useRealityContext();
	const parent = useParentContext();
	const instanceRef = (0, import_react.useRef)(new EntityRef(null, ctx));
	...
	(0, import_react.useEffect)(() => {
		...
		const init = async () => {
			try {
				...
				if (parent) {
					if (!(await parent.addEntity(ent)).success) throw new Error("parent.addEntity failed");
				} else if (!(await ctx.reality.addEntity(ent)).success) throw new Error("ctx.reality.addEntity failed");
	...
	useEntityId({
		id,
		entity: instanceRef.current.entity
	});
	...
	(0, import_react.useEffect)(() => {
		...
		if (!ent) return;
		if (enableInput !== void 0) ent.enableInput = !!enableInput;
...
var BaseEntity = (0, import_react.forwardRef)(({ children, createEntity, recreateKey, ...rest }, ref) => {
	const ctx = useRealityContext();
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ParentContext.Provider, {
		value: entity,
		children
	});
});
(0, import_react.forwardRef)((props, ref) => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BaseEntity, {
		...rest,
		id,
		ref,
		createEntity: async (ctxVal) => ctxVal.session.createEntity({
			id,
			name
		}),
		children
...
var GeometryEntity = (0, import_react.forwardRef)(({ id, children, name, materials, geometryOptions, createGeometry, ...rest }, ref) => {
	const ctx = useRealityContext();
	const entityRef = (0, import_react.useRef)(null);
	const componentRef = (0, import_react.useRef)(null);
	const mutableRef = (0, import_react.useRef)({
		lastSnapshot: null,
		rebuildGen: 0
	});
	(0, import_react.useEffect)(() => {
		...
		const geometryChanged = !shallowEqualObject(lastSnapshot.geometryOptions, geometryOptions);
		const materialsChanged = !shallowEqualArray(lastSnapshot.materials, materials);
		if (!geometryChanged && !materialsChanged) return;
		mutableRef.current.lastSnapshot = {
			geometryOptions,
			materials
		};
		...
		const rebuild = async () => {
			...
			try {
				...
				const materialList = await Promise.all(materials?.map((mid) => ctx.resourceRegistry.get(mid)).filter(Boolean) ?? []);
				if (gen !== mutableRef.current.rebuildGen) {
					await geometry.destroy();
					return;
				}
				...
				if (gen === mutableRef.current.rebuildGen) console.error("GeometryEntity: rebuild failed", error);
			}
		};
		rebuild();
	}, [
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BaseEntity, {
		...
		createEntity: async (ctx2, signal) => {
			...
			try {
				const ent = await manager.addResource(() => ctx2.session.createEntity({
					id,
					name
				}));
				const geometry = await manager.addResource(() => createGeometry(geometryOptions));
				const materialList = await Promise.all(materials?.map((id2) => ctx2.resourceRegistry.get(id2)).filter(Boolean) ?? []);
				const modelComponent = await manager.addResource(() => ctx2.session.createModelComponent({
					mesh: geometry,
					materials: materialList
				}));
...
(0, import_react.forwardRef)(({ children, ...props }, ref) => {
	const ctx = useRealityContext();
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(GeometryEntity, {
		...props,
		ref,
...
(0, import_react.forwardRef)(({ children, ...props }, ref) => {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(GeometryEntity, {
		...
		geometryOptions: { radius: props.radius },
		children
	});
});
(0, import_react.forwardRef)(({ children, ...props }, ref) => {
...
(0, import_react.forwardRef)(({ id, model, children, name, materials, ...rest }, ref) => {
	const ctx = useRealityContext();
	const entityRef = (0, import_react.useRef)(null);
	const lastMaterialsRef = (0, import_react.useRef)(void 0);
	(0, import_react.useEffect)(() => {
		if (!ctx || !entityRef.current) return;
		const next = materials ?? [];
		if (shallowEqualArray(lastMaterialsRef.current ?? [], next)) return;
		...
		const apply = async () => {
			try {
				...
				if (entityRef.current) await entityRef.current.setMaterials(materialList);
			} catch (error) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BaseEntity, {
		...
		createEntity: async (ctx2, signal) => {
			try {
				...
				if (!modelAsset) throw new Error(`ModelEntity: model not found ${model}`);
				...
				if (materials && materials.length > 0) {
					const materialList = (await Promise.all(materials.map((mid) => ctx2.resourceRegistry.get(mid)))).filter(Boolean);
					if (materialList.length > 0 && !signal.aborted) await ent.setMaterials(materialList);
				}
...
(0, import_react.forwardRef)(function RealityBase({ children, ...inProps }, ref) {
	if (useInsideAttachment()) {
		console.warn("[WebSpatial] Reality cannot be used inside AttachmentAsset.");
		return null;
	}
	const { onSpatialTap, onSpatialDragStart, onSpatialDrag, onSpatialDragEnd, onSpatialRotate, onSpatialRotateEnd, onSpatialMagnify, onSpatialMagnifyEnd, ...props } = inProps;
	const ctxRef = (0, import_react.useRef)(null);
	const creationId = (0, import_react.useRef)(0);
	const [isReady, setIsReady] = (0, import_react.useState)(false);
	const cleanupReality = (0, import_react.useCallback)(() => {
	...
	const createReality = (0, import_react.useCallback)(async () => {
		...
		try {
			if (!(await session.getSpatialScene().addSpatializedElement(reality)).success || isCancelled()) {
				resourceRegistry.destroy();
				attachmentRegistry.destroy();
				reality.destroy();
				return null;
	...
	const content = (0, import_react.useCallback)(() => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, {}), []);
	useRealityEvents({
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(RealityContext.Provider, {
		value: ctxRef.current,
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpatializedContainer, {
			component: "div",
			ref,
...
function ModelBase(props, ref) {
	...
	if (!enableXR || !spatial2.runInSpatialWeb() || insideAttachment) {
		const { onSpatialTap, onSpatialDragStart, onSpatialDrag, onSpatialDragEnd, onSpatialRotate, onSpatialRotateEnd, onSpatialMagnify, onSpatialMagnifyEnd, spatialEventOptions: _spatialEventOptions, ...modelProps } = restProps;
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("model", {
			ref,
			...modelProps
		});
	}
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SpatializedStatic3DElementContainer, {
		ref,
		...restProps
	});
}
var Model = withSSRSupported((0, import_react.forwardRef)(ModelBase));
Model.displayName = "Model";
if (typeof window !== "undefined") initPolyfill();
//#endregion
//#region node_modules/.pnpm/@webspatial+react-sdk@1.6.1_@webspatial+core-sdk@1.6.1_react-dom@19.2.6_react@19.2.6__react@19.2.6/node_modules/@webspatial/react-sdk/dist/jsx/jsx-dev-runtime.js
(function() {
	if (typeof window === "undefined") return;
	if (!window.__webspatialsdk__) window.__webspatialsdk__ = {};
	window.__webspatialsdk__["react-sdk-version"] = "1.6.1";
...
function jsx(type, props, key) {
	...
	return import_jsx_runtime.jsx(type, props, key);
}
...
//# sourceMappingURL=@webspatial_react-sdk_jsx-dev-runtime.js.map